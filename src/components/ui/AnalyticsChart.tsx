...
const yAxisProps = {
    ...
    const brushEl =
      showBrush || data.length > 60 ? (
        <Brush
          ...
          onChange={(range) =>
            setBrushIndex(range as { startIndex?: number; endIndex?: number })
          }
        />
      ) : null;
  ...
  const trendIcon =
    avg !== undefined && data.length > 1
      ? (() => {
          const lastVal = data[data.length - 1]?.[dataKey];
          if (typeof lastVal === "number") {
            if (lastVal > avg * 1.02)
              return <TrendingUp size={14} className="text-[#059669]" />;
            if (lastVal < avg * 0.98)
              return <TrendingDown size={14} className="text-[#DC2626]" />;
          }
        ...
        <div className="px-6 pt-5 pb-4 border-b border-[#F1F5F9]">
          <div className="flex items-start justify-between">
            ...
            <ChartActions
              ...
              onReset={
                brushIndex.startIndex !== undefined ? handleReset : undefined
              }
          ...
          >
            <div className="flex items-center gap-8 text-[12px]">
              {min !== undefined && (
                ...
                >
                  <span className="text-text-tertiary font-semibold uppercase tracking-wider">
                    Min
                  </span>
